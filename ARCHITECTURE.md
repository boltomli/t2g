...
│  4. 生成游戏文件:                                            │
│     - Godot项目 (.gd, .tscn, project.godot)                 │
│     - Twine故事 (.twee → 编译为 HTML)                       │
│  5. 用户运行: Godot打开项目 / 浏览器打开HTML                 │
└─────────────────────────────────────────────────────────────┘
```
...
**Godot 类型 (rpg/adventure/visual_novel/strategy/action):**
- 场景结构 (.tscn)
- GDScript代码 (.gd)
- 资源文件 (图片、音效占位)
- 游戏配置 (project.godot)
...
**Twine 类型 (twine):**
- Twee 源文件 (.twee)，使用 Chapbook 格式
- 包含: 配置、样式、脚本、段落（章节/结局/知识碎片）
- 编译为单文件 HTML 即可运行
...
└── pi_mode/                 # Pi Agent模式
    ...
    ├── generate.py          # 游戏生成入口
    ├── generators/          # 游戏类型生成器
    │   ├── visual_novel.py  # Godot 视觉小说生成器
    │   └── twine.py         # Twine 交互式故事生成器
    └── README.md
```
